...
const ViewProperty = () => {
	...
	const seoSlug = property?.title
		?.toLowerCase()
		.replace(/\s+/g, "-")
		.replace(/[^\w-]+/g, "");
	const pageUrl = `https://imperiagrouponline.com/properties/${seoSlug}`;

	return (
		<>
			<SEO
				title={`${property.title} in ${property.location} | Imperia Consulting`}
				description={property.desc?.slice(0, 150)}
				keywords={`property ${property.location}, ${property.title}, Nairobi real estate, Kenya apartments, ${property.avgPrice}`}
				ogImage={property.mainImg}
				ogUrl={pageUrl}
				canonicalUrl={pageUrl}
				isBlogPost={true}
				author='Imperia Consulting'
				...
				<div className='flex flex-col lg:flex-row lg:justify-between lg:items-start gap-8 mb-6'>
					...
					<div className='lg:w-80 flex-shrink-0'>
						<div className='bg-gray-50 p-6 rounded-xl border'>
							...
							<button className='w-full bg-primary-red hover:bg-blue-300 text-white font-semibold py-3 px-4 rounded-lg transition-colors duration-200 mb-4'>
								<a
									href={WALink.link}
									target='_blank'
									rel='noreferrer'
